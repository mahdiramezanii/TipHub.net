...
fc.lang = function(langCode, newFcOptions) {
	...
	// so no way to tell if this is an initialization or a default-settings.
...
// When settings the time, and the moment has an ambiguous time, it then becomes unambiguous.
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// Responsible for settings rowCnt/colCnt and any other row/col data
...
Grid.mixin({
	...
	documentDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this settings is on
			el = $(ev.target);
...
var TimeGrid = Grid.extend({
	...
	renderSelection: function(range) {
		if (this.view.opt('selectHelper')) { // this settings signals that a mock helper event should be rendered
...
var View = fc.View = Class.extend({
	...
	// Handler for unselecting when the user clicks something and the 'unselectAuto' settings is on
	documentMousedown: function(ev) {
		var ignore;
	...
	// `weekNumberCalculation` settings.
